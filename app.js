...
function filterPrice(){
  ...
  filterBtns.forEach(function(btn){
    ...
    const price = e.currentTarget.dataset.id;
           const menuPrice = menu.filter((menuItem)=>{
              if(menuItem.price >= price)
              return menuItem;
          })
     
          displayMenuItems(menuPrice);
        });
  });
}
...
function filterButtons (){
    ...
    const buttonCategories = categories.map(function(category){
       
        return `<button class="filter-btn" type="button" data-id="${category}">${category}</button>`;
    }).join("");

    buttonSection.innerHTML = buttonCategories;
